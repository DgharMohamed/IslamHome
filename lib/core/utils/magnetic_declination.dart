...
class GeomagneticField {
  ...
  factory GeomagneticField.calculate({
    ...
    final p = List.generate(
        maxN + 2, (i) => List.filled(maxN + 2, 0.0)); // P[n][m]
    final dp = List.generate(
        maxN + 2, (i) => List.filled(maxN + 2, 0.0)); // dP/dTheta
    ...
    for (int n = 2; n <= maxN; n++) {
      ...
      for (int m = 0; m <= n - 2; m++) {
        ...
        final k2 =
            math.sqrt(((nn - 1) * (nn - 1) - mm * mm) / (nn * nn - mm * mm));
    ...
    for (final coeff in _wmm2020) {
      ...
      bx += rRatio * (g * math.cos(m * lonRad) + h * math.sin(m * lonRad)) *
          dp[n][m];
