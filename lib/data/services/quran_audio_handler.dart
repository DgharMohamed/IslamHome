...
import 'package:islam_home/data/models/playback_session.dart';
import 'package:islam_home/data/services/playback_session_service.dart';
...
class QuranAudioHandler extends BaseAudioHandler
  ...
  Future<void> skipToNext() async {
    ...
    if (sequence.isEmpty) {
      debugPrint('🎵 QuranAudioHandler: No sequence to navigate');
      return;
    }
    ...
    if (!hasNext) {
      debugPrint(
        '🎵 QuranAudioHandler: Already at last item (${currentIndex + 1}/${sequence.length})',
      );
    ...
    // Update MediaItem for the new index
    final newIndex = _player.currentIndex ?? currentIndex;
    if (newIndex < sequence.length) {
      final tag = sequence[newIndex].tag;
      if (tag is MediaItem) {
        _updateMediaItem(tag);
        debugPrint(
          '🎵 QuranAudioHandler: Navigated to item ${newIndex + 1}/${sequence.length}: ${tag.title}',
        );
      }
  ...
  Future<void> skipToPrevious() async {
    ...
    if (!hasPrevious) {
      debugPrint(
        '🎵 QuranAudioHandler: Already at first item (${currentIndex + 1}/${sequence.length})',
      );
  ...
  Future<void> skipToQueueItem(int index) async {
    ...
    if (index < 0 || index >= sequence.length) {
      debugPrint(
        '🎵 QuranAudioHandler: Invalid index $index (sequence size: ${sequence.length})',
      );
      return;
    }
    ...
    if (tag is MediaItem) {
      _updateMediaItem(tag);
      debugPrint(
        '🎵 QuranAudioHandler: Navigated to item ${index + 1}/${sequence.length}: ${tag.title}',
      );
    }
  ...
  Future<void> setPlaylist(
    ...
    if (mediaItems.isNotEmpty) {
      ...
      // Try to populate _currentPlaylist for session persistence if these are Quran verses
      try {
        _currentPlaylist = mediaItems.map((item) {
          // Check if item has Quran metadata to convert back to QuranVerse
          if (item.extras != null &&
              item.extras!.containsKey('surahNumber') &&
              item.extras!.containsKey('verseNumber')) {
            return QuranVerse.fromMediaItem(item);
          }
          // Placeholder or minimal verse if full data not available but tagged as Quran
          return QuranVerse(
            surahNumber: item.extras?['surahNumber'] ?? 0,
            surahName:
                item.album?.replaceFirst('سورة ', '') ??
                item.title.split(' - ').first,
            verseNumber: item.extras?['verseNumber'] ?? 0,
            arabicText: item.extras?['arabicText'] ?? '',
            audioUrl: item.id,
            duration: item.duration ?? Duration.zero,
          );
        }).toList();
        debugPrint(
          '🎵 QuranAudioHandler: Reconstructed _currentPlaylist with ${_currentPlaylist.length} items',
        );
      } catch (e) {
        debugPrint(
          '🎵 QuranAudioHandler: Could not reconstruct _currentPlaylist: $e',
        );
        // If it's not Quran content (e.g. YouTube audio), clear the playlist to avoid inconsistent state
        _currentPlaylist = [];
      }
    }
